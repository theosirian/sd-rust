...
impl Shared {
	fn new() -> Self { Shared { subscribers: HashMap::new(), } }
}
...
impl Subscriber {
	fn new(subscriptions: Vec<sd_rust::EventType>,
	       state: Arc<Mutex<Shared>>,
	       frames: Frames)
	       -> Subscriber {
...
fn main() {
	...
	'input: loop {
		...
		match buf[0] {
			...
			b'3' => {
				send(sd_rust::EventData::PointerDown { pointer: 0,
				                                       button: 0,
				                                       x: 16,
				                                       y: 16, }.bytes());
			...
			b'4' => {
				send(sd_rust::EventData::PointerUp { pointer: 0,
				                                     button: 0,
				                                     x: 32,
				                                     y: 32, }.bytes());
			...
			b'5' => {
				send(sd_rust::EventData::PointerMove { pointer: 0,
				                                       button: 0,
				                                       x: 24,
				                                       y: 24, }.bytes());
